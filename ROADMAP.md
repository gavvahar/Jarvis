...
- [ ] **Device-camera lock (Sentry Mode add-on)** — while Sentry Mode is ARMED, use the *device's own webcam* (the laptop/desktop JARVIS is open on, via client-side `getUserMedia` — not a network camera) to detect who's in front of the screen; if a face is matched that isn't the logged-in user's *and* the logged-in user's own face isn't visible in the same frame, lock the JARVIS session in that browser tab. Scoped as an in-app lock only — no browser exposes a real OS-level lock-screen API to a webpage, so this can't trigger an actual Windows/Linux/macOS lock; it blanks/hides the JARVIS UI and requires a face re-scan (or password fallback) to resume. Being pure client-side web APIs (no OS-specific code), it works identically cross-platform (Linux, Windows, macOS, mobile browsers) — though it's most useful on laptops/desktops where the tab stays open unattended, since phones already have their own lock screen.

**Device-camera lock — planned build order** (safest first, each item its own commit):

1. **Presence-check endpoint** — new `POST /api/face/check-presence` in `python/app.py`, thin wrapper around the already-existing `_identify_faces_in_image` (`python/integrations/vision.py:110-130`) and the household `_face_cache` populated by `_refresh_face_cache` (`vision.py:133-136`) — accepts an uploaded frame, returns every matched face in it (`[{user_id, name, confidence}]`), same shape `_vision_loop` already produces per camera. No new face-matching logic needed, just exposes the existing per-frame multi-face match as a route.
2. **Client capture loop** — new `static/v2/js/app/sentry_lock.js`: requests `getUserMedia({video: true})` into a hidden `<video>`, only starts capturing when Sentry Mode is `armed` (listen for the existing `sentry_mode_changed` socket event) *and* the tab is visible (Page Visibility API) *and* the logged-in user has an enrolled face. Captures a frame to a `<canvas>` every ~5s, POSTs to `/api/face/check-presence`.
3. **Consecutive-mismatch state machine** — lock condition is: the logged-in user's `user_id` is *absent* from the matched faces *and* at least one *other* matched (or unmatched/`unknown`) face *is* present, for 3 consecutive checks (~15s) — a single bad frame (bad lighting, brief look-away) doesn't trigger it; the counter resets on any check where the logged-in user is visible again.
4. **Lock screen UI** — full-viewport overlay (new `static/v2/css/sentry_lock.css`) blanking the rest of the app, "JARVIS LOCKED" state, continues running the same capture loop underneath to auto-unlock the moment the logged-in user's face reappears alone. Fallback escape hatch is mandatory (face matching isn't infallible): a "unlock with password" option that re-runs the normal login/session check, so a legitimate user can never be permanently locked out by a bad camera angle.
5. **Snapshot on lock event** — reuse the `security_events` table (new `event_type: "device_lock"`) so a lock event shows up in the same RECENT EVENTS list Sentry Mode already has, with the triggering snapshot attached via the existing `_db_record_security_event(..., snapshot=...)` path.
