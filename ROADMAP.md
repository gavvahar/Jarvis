...
- [ ] **Device-camera lock (Sentry Mode add-on)** — while Sentry Mode is ARMED, use the *device's own webcam* (the laptop/desktop JARVIS is open on, via client-side `getUserMedia` — not a network camera) to detect who's in front of the screen; if a face is matched that isn't the logged-in user's *and* the logged-in user's own face isn't visible in the same frame, lock the JARVIS session in that browser tab. Scoped as an in-app lock only — no browser exposes a real OS-level lock-screen API to a webpage, so this can't trigger an actual Windows/Linux/macOS lock; it blanks/hides the JARVIS UI and requires a face re-scan (or password fallback) to resume. Being pure client-side web APIs (no OS-specific code), it works identically cross-platform (Linux, Windows, macOS, mobile browsers) — though it's most useful on laptops/desktops where the tab stays open unattended, since phones already have their own lock screen. Broken into 5 independently shippable parts below (each its own commit/PR, buildable and testable in isolation):
  ...
  - [ ] **Part 2 — Client capture loop** — new `static/v2/js/app/sentry_lock.js`: requests `getUserMedia({video: true})` into a hidden `<video>`, only starts capturing when Sentry Mode is `armed` (listen for the existing `sentry_mode_changed` socket event) *and* the tab is visible (Page Visibility API) *and* the logged-in user has an enrolled face. Captures a frame to a `<canvas>` every ~5s, POSTs to Part 1's endpoint, logs the result to the console. Depends on Part 1; ships with no visible UI change yet (just console output) so it can be verified in isolation before the lock behavior exists.
  - [ ] **Part 3 — Consecutive-mismatch state machine** — lock condition is: the logged-in user's `user_id` is *absent* from the matched faces *and* at least one *other* matched (or unmatched/`unknown`) face *is* present, for 3 consecutive checks (~15s) — a single bad frame (bad lighting, brief look-away) doesn't trigger it; the counter resets on any check where the logged-in user is visible again. Depends on Part 2; still no visible lock UI — exposes a `shouldLock()` boolean the next part consumes, unit-testable on its own with fake check sequences.
