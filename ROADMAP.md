...
1. **Email connection layer (foundational)** — a generic **IMAP** connection (not a Gmail-specific OAuth app), matching the existing CalDAV/CardDAV pattern (`python/integrations/pim/dav.py` + the PIM settings panel): works with Gmail, iCloud, Fastmail, etc. via an app-specific password, no per-provider OAuth app to register. New `python/integrations/pim/mail.py` with an `_imap_fetch_unread(config, limit)` helper (Python's stdlib `imaplib`, read-only `SELECT INBOX` + `SEARCH UNSEEN`, fetch headers + a plain-text snippet), an **EMAIL** section in the PIM settings panel (`email_imap_url`/`email_username`/`email_password` columns on `user_configs`, mirroring `calendar_url`/`calendar_username`/`calendar_password`), and a bare `list_unread_email` voice tool that just lists subjects/senders — no classification yet. This step alone proves connectivity end-to-end before any LLM logic is layered on.
2. **Email triage — classify & summarize** — a poll loop (`_email_triage_loop()`, same shape as `_briefing_loop()`) that fetches unread mail via step 1, runs each new message through a cheap LLM pass to get a one-line summary + importance flag, and caches results in a new `email_triage` table keyed by IMAP UID so a message is never re-classified. Surfaced via a `get_email_summary` voice tool and an **EMAIL TRIAGE** list in the settings panel (subject, one-line summary, importance badge). Opt-in per user (`email_triage_enabled`, off by default), same convention as `briefing_enabled`/`habit_nudges_enabled`.
3. **Email triage — urgent alerts** — layered on step 2: when a newly-classified message comes back "urgent," fire an `email_alert` socket event + push through the existing `_send_push` fan-out, the same delivery pattern every other proactive alert in this phase already uses. No new infra, just a conditional branch in the step-2 loop.
4. **Package tracking** — also layered on step 1's IMAP connection, independent of steps 2–3: a separate lightweight pass over unread mail matching known shipping-carrier senders/subjects (UPS, FedEx, USPS, Amazon) via regex, extracting a tracking number and delivery-estimate line where present, and announcing a `package_alert` (delivered/out-for-delivery) through the same push fan-out. Carrier-specific tracking-status APIs (vs. just parsing the email text) are a possible follow-up, not required for v1.
