...
3. **Motion-only detection** — `_vision_loop` (`vision.py:258-338`) currently does `if not detections: continue` (`vision.py:286-287`), so a frame with zero faces produces nothing. Add a module-level `_last_frame_cache: dict[camera_id, ndarray]` of small grayscale frames; on each poll, decode the snapshot, diff it against the cached frame for that camera (OpenCV frame differencing — `opencv-python-headless` is already a dependency, no new package needed), and if the mean diff exceeds a threshold *and* no face was matched *and* sentry mode is `armed` or (`auto` and away/night), record a `motion` security event instead of silently skipping.
