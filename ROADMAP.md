...
**Sentry Mode — planned build order** (safest first, each item its own commit):

1. **Explicit arm/disarm state** — new singleton table `sentry_state (id BIGINT PRIMARY KEY CHECK (id=1), mode TEXT NOT NULL DEFAULT 'auto', updated_by TEXT REFERENCES user_configs(user_id), updated_at TIMESTAMPTZ)`, one row for the whole household (presence/away-mode is already computed globally across all users in `_vision_loop`, not per-user, so the arm state should match that scope rather than living on `user_configs`). `mode` is `'auto'` (today's behavior: heightened detection only during away-mode/night), `'armed'` (always heightened, regardless of presence/time — the Tesla-style override), or `'disarmed'` (never fire security events, e.g. cleaners are over). New `GET/POST /api/sentry-mode` endpoints in `python/app.py`; `POST` validates the enum, updates the row, and broadcasts a `sentry_mode_changed` socket event to all connected sids (iterate `_sid_to_user`, not just one user's sids) so every logged-in household member's UI stays in sync.
2. **Voice control tool** — new `python/integrations/sentry.py` mirroring the existing per-module tool pattern (`VISION_TOOLS_ANTHROPIC` / `_get_vision_tools` / `_execute_vision_tool` in `vision.py:162-255`): a `set_sentry_mode` tool (`args: {mode: "armed"|"disarmed"|"auto"}`) and a `get_sentry_mode` tool. Register in `python/llm.py` next to the existing `+ _get_vision_tools(provider)` call (`llm.py:340`) and add dispatch (either its own `elif block.name == "set_sentry_mode"` branch alongside `manage_device_alert` etc. at `llm.py:388`, or a `_SENTRY_TOOL_NAMES` set check inside `_execute_ha_tool` following the `_VISION_TOOL_NAMES` pattern at `llm.py:60`).
3. **Motion-only detection** — `_vision_loop` (`vision.py:258-338`) currently does `if not detections: continue` (`vision.py:286-287`), so a frame with zero faces produces nothing. Add a module-level `_last_frame_cache: dict[camera_id, ndarray]` of small grayscale frames; on each poll, decode the snapshot, diff it against the cached frame for that camera (OpenCV frame differencing — `opencv-python-headless` is already a dependency, no new package needed), and if the mean diff exceeds a threshold _and_ no face was matched _and_ sentry mode is `armed` or (`auto` and away/night), record a `motion` security event instead of silently skipping.
4. **Snapshot capture on trigger** — `_db_record_security_event` (`db.py:582-591`) already accepts a `snapshot: bytes | None` and the column exists in `schema.sql:172-180`, but every call site passes none. Thread the raw JPEG bytes already captured in `_vision_loop` (from `_get_ha_camera_snapshot`/`_capture_rtsp_frame`) through to both the existing `unknown_person` call site (`vision.py:318`) and the new `motion` call site from step 3. Add `GET /api/security-events/{id}/snapshot` (new `_db_get_security_event_snapshot(user_id, event_id)` in `db.py`) returning `image/jpeg` bytes. True video clips are out of scope for this pass — the polling interval (`VISION_POLL_INTERVAL`, default 30s) and lack of a continuous recording buffer make a real clip a separate, much larger effort (would need continuous RTSP capture into a ring buffer); a still snapshot per event is the realistic MVP.
5. **Frontend — Sentry Mode panel** — new "SENTRY MODE" section in `templates/partials/vision_settings_panel.html`, slotted between "CAMERAS" and "FACE ENROLLMENT" (matching the existing `.vision-section-label` block structure at `vision_settings_panel.html:8-30`): a 3-way AUTO/ARMED/DISARMED control wired via `fetch` to the step-1 endpoints (no existing frontend toggle-to-fetch pattern to copy verbatim — closest analog is the reload-after-write flow in `vision.js:39-44` and `garage.js:60-76`), plus an event log that finally calls the already-unused `GET /api/security-events` endpoint (`vision.py:377`, confirmed dead code on the frontend today) and renders each event with an `<img src="/api/security-events/{id}/snapshot">` thumbnail — no base64/image-rendering pattern exists elsewhere in the frontend yet, so this is new. Socket handling for the new `sentry_mode_changed` event and any new alert copy follows the existing toast pattern in `static/v2/js/app/doorbell.js` (`showDoorbellToast` / the inline `security_alert` handler at `doorbell.js:69-97`).
6. **Push notification wiring** — depends on Phase 4 build-order item 3 ("Push notifications") shipping first, since that item already fully scopes the shared infrastructure (`push_subscriptions` table, `python/integrations/push.py`, VAPID env vars, `sw.js` `push`/`notificationclick` listeners). Once it exists, Sentry Mode just adds its `motion`/`unknown_person` emit sites to the same fan-out `_send_push` call other alert types use — no separate push design needed here.
