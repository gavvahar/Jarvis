...
- [x] **Visual TTS output** — display Jarvis's spoken response as text on screen simultaneously; never lose a reply because it was too quiet or came from the wrong direction. Audited first, not just built blind: `core.js` already rendered every reply into `window.__chat`, but that's the Chat Tab drawer (`chat.js`), hidden by default and only opened with `[C]` — during normal voice use, text was rendering into an invisible panel while speech played, a real gap. New always-visible `#caption-bar` (`film_overlays.html`, styled in `accessibility.css`) mirrors the same `speak_sentence`/`response_done` events in `accessibility.js`, independent of the chat drawer's open state; fades out ~2.5s after the reply finishes. Doubles as the caption surface the still-pending conversation/media-caption items will reuse.
- [x] **Wake word visual confirmation** — flash a light or show an on-screen indicator when the wake word fires, so it's clear Jarvis heard you. Reused rather than rebuilt: `standby.js` already had an unused-outside-mode-changes `#mode-flash` energy-surge (`triggerFlash()`, `mode_switching.css`'s `modeFlash` keyframe — a single 0.8s cyan radial pulse, no strobe), now exposed as `window.__triggerWakeFlash()`. New `static/v2/js/app/accessibility.js` calls it on every `wake_trigger` socket event unconditionally — `doorbell.js`'s existing handler only acks/wakes when starting from standby, so a repeat wake word while already awake previously got zero feedback; now it always flashes.
...
- [x] **Adjustable TTS clarity** — per-user controls for Jarvis voice speed, volume, and EQ; default to slower and louder for the hearing-impaired profile. Opt-in, not a new global default (columns default to today's `1.0/1.0/1.0`; each user chooses their own) — `schema.sql`'s `tts_rate`/`tts_pitch`/`tts_volume` on `user_configs`, `db.py`'s `_db_get_tts_prefs`/`_db_set_tts_prefs`, `GET`/`POST /api/tts-prefs` (values also included in `/api/status` since `core.js` needs them before any settings panel is ever opened). New ACCESSIBILITY tab (`accessibility_settings_panel.html`) with rate/pitch/volume sliders, a one-click "hearing-assist preset" (0.8x rate — volume is already at the Web Speech API's ceiling, so there's no "louder" to give beyond that), wired in `accessibility.js`; `core.js`'s `_ttsNext()` now reads `_ttsRate/_ttsPitch/_ttsVolume` instead of hardcoded `1.0`s, set via a new exported `setTtsPrefs()`.
