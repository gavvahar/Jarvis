...
(function () {
  ...
  new MutationObserver(() => { if (!isOnline() && open) setOpen(false); })
  ...
  function addMsg(text, who) {
    ...
    meta.textContent = (who === 'out' ? 'YOU' : 'J.A.R.V.I.S') + ' · ' + nowTime();
  ...
  function updateMsg(m, text) {
    if (!m) return;
    const b = m.querySelector('.bubble'); if (b) b.textContent = text;
    log.scrollTop = log.scrollHeight;
  }
