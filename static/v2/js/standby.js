...
(function () {
  ...
  g.appendChild(
    el("circle", { cx: CX, cy: CY, r: 452, class: "sb-thin-faint" }),
  );
  for (let i = 0; i < 150; i++) {
    const a = (i / 150) * TAU;
    const long = i % 5 === 0;
    const [x0, y0] = pol(456, a),
      [x1, y1] = pol(long ? 470 : 464, a);
    g.appendChild(
      el("line", {
        x1: x0,
        y1: y0,
        x2: x1,
        y2: y1,
        class: long ? "sb-tick" : "sb-tick-faint",
      }),
    );
  }
  ...
  for (let i = 0; i < 96; i++) {
    ...
    const a = (i / 96) * TAU;
    const [x0, y0] = pol(402, a),
      [x1, y1] = pol(434, a);
    g.appendChild(
      el("line", { x1: x0, y1: y0, x2: x1, y2: y1, class: "sb-tooth" }),
    );
  }
  ...
  [
    ...
    g.appendChild(
      el("path", { d: arc(418, s[0] * P, s[1] * P), class: "sb-arc" }),
    );
  });
  // thin echo arcs just inside
  g.appendChild(
    el("path", { d: arc(404, 0.4 * P, 0.66 * P), class: "sb-arc-thin" }),
  );
  g.appendChild(
    el("path", { d: arc(404, 1.3 * P, 1.78 * P), class: "sb-arc-thin" }),
  );

  // ---------- (4) YELLOW ACCENT arc on the LEFT (rotates clockwise with main arcs, pulses) ----------
  g = G(1.0);
  g.appendChild(
    el("path", { d: arc(418, 0.82 * P, 1.12 * P), class: "sb-yellow" }),
  );
  [0.82, 1.12].forEach((m) => {
    const [x0, y0] = pol(406, m * P),
      [x1, y1] = pol(430, m * P);
    g.appendChild(
      el("line", { x1: x0, y1: y0, x2: x1, y2: y1, class: "sb-yellow-cap" }),
    );
  });

  // ---------- (5) STATUS DOTS along the top (slow CW) ----------
  g = G(0.07);
  const dotAngs = [-0.6, -0.52, -0.44, -0.36, -0.28];
  dotAngs.forEach((m, i) => {
  ...
  for (let i = 0; i < 72; i++) {
    const a = (i / 72) * TAU;
    const big = i % 6 === 0;
    const [x0, y0] = pol(336, a),
      [x1, y1] = pol(big ? 320 : 326, a);
    g.appendChild(
      el("line", {
        x1: x0,
        y1: y0,
        x2: x1,
        y2: y1,
        class: big ? "sb-tick" : "sb-tick-faint",
      }),
    );
  }
  g.appendChild(
    el("path", { d: arc(344, 0.18 * P, 0.62 * P), class: "sb-arc-thin" }),
  );
  ...
  [
    ...
    g.appendChild(
      el("path", { d: arc(300, s[0] * P, s[1] * P), class: "sb-hub" }),
    ),
  );
  g.appendChild(
    el("path", { d: arc(276, 0.55 * P, 1.45 * P), class: "sb-hub" }),
  );
  g.appendChild(
    el("path", { d: arc(276, 1.55 * P, 2.45 * P), class: "sb-hub" }),
  );
  g.appendChild(
    el("circle", { cx: CX, cy: CY, r: 252, class: "sb-hub-faint" }),
  );
  for (let i = 0; i < 12; i++) {
    const a = (i / 12) * TAU;
    const [x0, y0] = pol(252, a),
      [x1, y1] = pol(300, a);
    g.appendChild(
      el("line", { x1: x0, y1: y0, x2: x1, y2: y1, class: "sb-hub-spoke" }),
    );
  }
