...
(function () {
  ...
  seg.forEach((s, i) => {
    g.appendChild(
      el("path", {
        d: arc(478, s[0] * Math.PI, s[1] * Math.PI),
        class: i % 4 === 0 ? "r-amber" : "r-stroke",
      }),
    );
  });
  // outer fine ticks
  ...
  const scanArc = el("path", {
    d: arc(398, -0.18 * Math.PI, 0.18 * Math.PI),
    class: "r-amber scan",
  });
  ...
  [
    ...
    g.appendChild(
      el("path", {
        d: arc(360, s[0] * Math.PI, s[1] * Math.PI),
        class: "r-stroke",
      }),
    );
  });
  ...
  for (let i = 0; i < 24; i++) {
    ...
    g.appendChild(
      el("rect", {
        ...
        fill: i % 6 === 0 ? "rgba(255,182,72,.7)" : "none",
      }),
    );
  }
  ...
  for (let i = 0; i < 72; i++) {
    ...
    const [x0, y0] = pol(r0, a),
      [x1, y1] = pol(r1, a);
    g.appendChild(
      el("line", { x1: x0, y1: y0, x2: x1, y2: y1, class: "tick-faint" }),
    );
  }
  const tArc = el("path", {
    d: arc(308, 0.6 * Math.PI, 1.4 * Math.PI),
    class: "r-stroke",
  });
  tArc.setAttribute("stroke-width", "2");
  tArc.setAttribute("opacity", "0.85");
  g.appendChild(tArc);
  ...
  [0, 0.5, 1, 1.5].forEach((m) => {
    const a = m * Math.PI;
    const [x0, y0] = pol(264, a),
      [x1, y1] = pol(280, a);
    g.appendChild(
      el("line", { x1: x0, y1: y0, x2: x1, y2: y1, class: "tick" }),
    );
  });
  ...
  function renderHud() {
    ...
    if (t.cpu != null) {
      setText("val-cpu", t.cpu + "%");
      setFill("fill-cpu", t.cpu);
    }
    if (t.ram != null) {
      setText("val-ram", t.ram + "%");
      setFill("fill-ram", t.ram);
    }
    ...
    if (t.gpu_mem_used != null && t.gpu_mem_total)
      setText(
        "val-vram",
        (t.gpu_mem_used / 1024).toFixed(1) +
          "/" +
          (t.gpu_mem_total / 1024).toFixed(0) +
          " GB",
      );
    ...
    setText(
      "val-latency",
      t.net_latency_ms != null && t.net_latency_ms >= 0
        ? num(t.net_latency_ms, 0)
        : "—",
    );
  }
  ...
  setInterval(renderWeather, 1500);
  renderWeather();
  ...
  setInterval(() => {
    ...
    const v = Math.min(
      99.9,
      Math.max(gpu, gpu * 0.5 + act * 55 + (t.infer_active ? 22 : 0)),
    );
  ...
  setInterval(() => {
    if (recogEl && window.__recognition != null)
      recogEl.textContent = window.__recognition;
  }, 200);
  ...
  if (spark) {
    ...
    const path = el("polyline", {
      ...
      style: "filter:drop-shadow(0 0 4px rgba(127,233,255,.6))",
    });
    const area = el("polygon", {
      fill: "rgba(127,233,255,0.08)",
      stroke: "none",
    });
    ...
    setInterval(() => {
      ...
      data.push(Math.min(1, d / peak));
      data.shift();
      const sp = data
        .map((v, i) => `${(i / (pts - 1)) * W},${H - v * H}`)
        .join(" ");
