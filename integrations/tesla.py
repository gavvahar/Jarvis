...
def _tesla_base_url(method: str) -> str:
    return _TESLA_OWNER_BASE if method == "unofficial" else _TESLA_FLEET_BASE


async def _tesla_access_token(method: str, user_id: str, config: dict) -> str:
    cached = _tesla_tokens.get(user_id, {})
    expiry = cached.get(f"{method}_expiry")
    if cached.get(f"{method}_access") and expiry and expiry > datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=5):
        return cached[f"{method}_access"]

    if method == "unofficial":
        refresh_field = "tesla_refresh_token"
        payload = {
        ...
        refresh_field = "tesla_fleet_refresh_token"
        payload = {
            "grant_type": "refresh_token",
    ...
    data = await refresh_oauth_token(f"{_TESLA_AUTH_BASE}/token", payload)
    ...
    _tesla_tokens[user_id].update({f"{method}_access": access_token, f"{method}_expiry": expiry_dt})
    if new_refresh and new_refresh != config.get(refresh_field):
        config[refresh_field] = new_refresh
        query = (
            "UPDATE user_configs SET tesla_refresh_token = $2 WHERE user_id = $1"
            if method == "unofficial"
            else "UPDATE user_configs SET tesla_fleet_refresh_token = $2 WHERE user_id = $1"
        )
        async with _pool().acquire() as conn:
            await conn.execute(query, user_id, new_refresh)
    return access_token


async def _tesla_vehicles(method: str, user_id: str, config: dict) -> list:
    token = await _tesla_access_token(method, user_id, config)
    async with httpx.AsyncClient(timeout=15) as c:
        r = await c.get(
            f"{_tesla_base_url(method)}/api/1/vehicles",
...
async def _tesla_wake(method: str, vehicle_id, token: str) -> bool:
    for _ in range(10):
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.post(
                f"{_tesla_base_url(method)}/api/1/vehicles/{vehicle_id}/wake_up",
                headers={"Authorization": f"Bearer {token}"},
            )
...
async def _tesla_cmd(method: str, user_id: str, config: dict, vehicle_id, command: str, data: dict | None = None) -> dict:
    token = await _tesla_access_token(method, user_id, config)
    await _tesla_wake(method, vehicle_id, token)
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(
            f"{_tesla_base_url(method)}/api/1/vehicles/{vehicle_id}/command/{command}",
            headers={"Authorization": f"Bearer {token}"},
            json=data or {},
        )
        r.raise_for_status()
...
async def _tesla_pick_vehicle(user_id: str, config: dict, name_hint: str | None = None) -> tuple:
    ...
    if method in ("unofficial", "both"):
        try:
            vehicles = await _tesla_vehicles("unofficial", user_id, config)
            if vehicles:
    ...
    vehicles = await _tesla_vehicles("fleet", user_id, config)
    if not vehicles:
...
async def _execute_tesla_tool(config: dict, name: str, args: dict, user_id: str = "") -> str:
    try:
        ...
        if method == "unofficial":
            vid = vehicle["id"]
            token = await _tesla_access_token("unofficial", user_id, config)
            ...
            if name == "set_climate":
                ...
                if action == "stop":
                    resp = await _tesla_cmd("unofficial", user_id, config, vid, "auto_conditioning_stop")
                else:
                    resp = await _tesla_cmd("unofficial", user_id, config, vid, "auto_conditioning_start")
                    temp_f = args.get("temperature_f")
                    if temp_f is not None:
                        temp_c = (float(temp_f) - 32) * 5 / 9
                        await _tesla_cmd("unofficial", user_id, config, vid, "set_temps", {"driver_temp": temp_c, "passenger_temp": temp_c})
            ...
            if name == "actuate_trunk":
                ...
                resp = await _tesla_cmd("unofficial", user_id, config, vid, "actuate_trunk", {"which_trunk": which})
            ...
            if name in _CMD:
                cmd, label = _CMD[name]
                resp = await _tesla_cmd("unofficial", user_id, config, vid, cmd)
                return f"{label} on {display}." if resp.get("result") else f"Command failed: {resp.get('reason', 'unknown')}"

        else:  # fleet
            vin = vehicle.get("vin", "")
            token = await _tesla_access_token("fleet", user_id, config)
            ...
            if name == "set_climate":
                ...
                await _tesla_cmd("fleet", user_id, config, vin, cmd)
                temp_f = args.get("temperature_f")
                if action == "start" and temp_f is not None:
                    temp_c = (float(temp_f) - 32) * 5 / 9
                    await _tesla_cmd("fleet", user_id, config, vin, "set_temps", {"driver_temp": temp_c, "passenger_temp": temp_c})
                return f"Climate {'started' if action == 'start' else 'stopped'} on {display}."

            if name == "actuate_trunk":
                which = args.get("which", "rear")
                await _tesla_cmd("fleet", user_id, config, vin, "actuate_trunk", {"which_trunk": which})
